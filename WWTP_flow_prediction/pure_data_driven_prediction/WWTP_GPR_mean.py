...
sensor data is used 
mean function taken from the SWMM model is used as the mean function of the GPR model
Author: Mohsen 
Date: 26/12/2025
"""
import pandas as pd 
...
def mean_function(mean_data, scaler_Y, timestep, average_inflow):
    ...
    the mean is returned to the model with multiplying to the average flow values of th training data
    """
...
def plot_results(timestamps_train, Y_train, Y_pred_train, std_train,
    ...
    ax.set_title(f'GPR: WWTP Inflow Prediction (Train: {train_days} days, Test: {test_hours} hours, mean func defined)', 
                 fontsize=14)
