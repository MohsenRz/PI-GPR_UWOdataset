""" 
making a simple XGBoost model for WWTP data prediction for comparison
...
def preparing_data(merged_data, start_time, interval_minutes=timeinterval, lag_minutes=120,
    ...
    # Exogenous formulation: only time and precipitation
    time_feat = np.array([(ts - start_time).total_seconds() / 60.0 for ts in timestamps]).reshape(-1, 1)
    X_multi = np.hstack((time_feat, sin_time, cos_time, rain_accum))
    Y = inflow
...
def plot_results(timestamps_train, Y_train, Y_pred_train,
    ...
    ax.set_title(f'XGBoost: WWTP Inflow Prediction (Train: {train_days} days, Test: {test_hours} hours)', 
                 fontsize=14)
...
def main():
    ...
    print("="*70)
    print("XGBoost Model for WWTP Inflow Prediction (Exogenous)")
    print("="*70)
