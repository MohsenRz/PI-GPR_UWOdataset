...
# constraints
min_tank_level = None # mm
max_tank_level = None # mm
mean_value = 100 # mm
...
def build_gpr_model(X_train, Y_train,time_std_dev, scaler_Y=None):
    ...
    ### rain kernel 
    kernel_rain = gpflow.kernels.Matern12(variance=2, active_dims=[1])
    bounded_transform_rain = tfp.bijectors.Sigmoid(
        low=tf.constant(0.1, dtype=tf.float64), 
        high=tf.constant(0.5, dtype=tf.float64)) 
    kernel_rain.lengthscales = gpflow.Parameter(0.4, transform=bounded_transform_rain)
    ...
    # freezing the mean function to the scaled mean value
...
def plot_results(timestamps_train, Y_train, Y_pred_train, std_train,
    ...
    ax.set_title(f'GPR: Tank Level Prediction, 2021 (Train: {train_days} days, Test: {test_hours} hours)', fontsize=14)
    ...
    # Shade region from first to last overflow point (based on actual values)
    overflow_mask = Y_test.ravel() > overflow_level
    overflow_indices = np.where(overflow_mask)[0]
    if len(overflow_indices) > 0:
        first_overflow_idx = overflow_indices[0]
        last_overflow_idx = overflow_indices[-1]
        axs[1].axvspan(timestamps_test[first_overflow_idx], timestamps_test[last_overflow_idx], 
                      alpha=0.15, color='red', label='Overflow Period (Actual)')
    ...
    axs[1].set_title(f'Probability of CSO (Tank Level > {overflow_level} mm) in the Prediction Zone', fontsize=14)
