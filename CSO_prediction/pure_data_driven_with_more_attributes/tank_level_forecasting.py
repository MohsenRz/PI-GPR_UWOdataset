...
sensor data is used 
this is a naive model that only takes Water level data for prediction
...
Tank_level_train = Tank_level[(Tank_level['timestamp'] >= train_start_time) & (Tank_level['timestamp'] <= train_end_time)]
precipitation_train = precipitation[(precipitation['timestamp'] >= train_start_time) & (precipitation['timestamp'] <= train_end_time)]

Tank_level_test = Tank_level[(Tank_level['timestamp'] >= train_end_time) & (Tank_level['timestamp'] <= test_end_time)]
precipitation_test = precipitation[(precipitation['timestamp'] >= train_end_time) & (precipitation['timestamp'] <= test_end_time)]

Tank_level_train = Tank_level_train.set_index('timestamp')
Tank_level_test = Tank_level_test.set_index('timestamp')
...
tank_level_train_resampled = Tank_level_train.resample(interval_string).mean().fillna(0)
tank_level_test_resampled = Tank_level_test.resample(interval_string).mean().fillna(0)
...
# Merge data 
merged_train = tank_level_train_resampled.join(precipitation_train_resampled, 
                                         lsuffix='_inflow', rsuffix='_precipitation', how='inner')
merged_test = tank_level_test_resampled.join(precipitation_test_resampled, 
                                       lsuffix='_inflow', rsuffix='_precipitation', how='inner')
...
def main():
    ...
    print("="*70 + "\nNaive Tank level GPR Prediction\n" + "="*70)
