"""
making a Gaussian Process Regression model for tank level prediction
sensor data is used 
this is a naive sparse model that only takes tank level data for predictions 
automatic lag time detection is used to find the optimal lag time for precipitation data in short term
sparsifiation is used for boosting the speed 
...
def build_sgpr_model(X_train, Y_train, time_std_dev, scaler_Y=None, threshold_scaled=None):
    """
    Build and train SGPR model with constraints 
    ...
    if scaler_Y is not None:
        ...
        print(f"Mean constraint: {mean_value} mm (scaled: {mean_scaled:.4f})")
    else:
    ...
    # freezing inducing points / allowing them to be optimised
...
def plot_results(timestamps_train, Y_train, Y_pred_train, std_train,
    ...
    ax.set_title(f'SGPR: Tank level prediction (Train: {train_days} days, Test: {test_hours} hours)', 
                 fontsize=14)
...
def plot_point_of_interest(target_date_str, timestamps, X_scaled, model, 
    ...
    Visualises the difference between Raw Mean, Truncated Mean, and Mode.
    ...
    ax.set_xlabel("Tank Level (mm)", fontsize=12)
...
def main():
    ...
    model = build_sgpr_model(X_train_scaled, Y_train_scaled, scaler_X.scale_[0], scaler_Y, threshold_scaled)
    ...
    #--- PLOTTING ---
    sample_date = None # str(train_end_time + pd.Timedelta(hours=24))  # it shows the 24th hour of the predictions
    plot_results(timestamps_train, Y_train, Y_pred_train, std_train_y,
                 ...
                 sample_date=sample_date, Z_timestamps=Z_timestamps, Z_values=Z_values)
    if sample_date is not None:
        plot_point_of_interest(sample_date, timestamps_test,
                           X_test_scaled, model, scaler_Y, Y_actual=Y_test,
                           min_val=min_tank_level, max_val=max_tank_level)
